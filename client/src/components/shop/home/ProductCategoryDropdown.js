...
const CategoryList = () => {
  ...
  return (
    <div className={`${data.categoryListDropdown ? "hidden" : ""} my-4 relative`}>
      <hr />
      ...
      return (
        <Fragment key={index}>
          ...
          >
            ...
            />
            <div className="categoryName">{item.cName}</div>
          </div>
        </Fragment>
      );
    })
  ) : (
    <div className="noCategory">No Category</div>
  )}
</div>
